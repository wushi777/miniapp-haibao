...
export class CommonFuncs {
    ...
    public static displayDateTime(mmSeconds: number): string {
        const result = moment(mmSeconds).format('YYYY-MM-DD HH:mm:ss');
        return result;
    }
    ...
    // 获取最近多长时间
    public static getRecentTime(num: number, type: any) {
    ...
    // 获取开始结束时间戳
    public static handleGetStartEndDate(timeNum: number, timeType: string) {
        const endDate   = Math.floor(Date.now());
        const startDate = endDate + this.getRecentTime(timeNum, timeType).asMilliseconds();
